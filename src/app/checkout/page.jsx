...
export default function CheckoutPage() {
  ...
  const placeOrder = async () => {
    ...
    // Prepare order data - make sure all required fields are sent
    const orderData = {
      items: cart.map((item) => ({
        product_id: item.product_id,
        quantity: item.quantity,
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          errorMessage = errorData.message || JSON.stringify(errorData.errors) || "Failed to place order";
        } catch (e) {
        ...
        <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="bg-white rounded-xl shadow-md p-6">
              ...
              <div className="space-y-3">
                ...
                <label className="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-gray-50 transition">
                  ...
                  <div className="flex-1">
                    <span className="font-semibold text-black">bKash</span>
                    <p className="text-sm text-black">Pay with bKash mobile banking</p>
                  </div>
                </label>
              ...
              {paymentMethod === "bkash" && (
                <div className="mt-4 p-4 bg-pink-50 rounded-lg">
                  <p className="text-sm text-pink-800">
                    <strong>Send payment to:</strong> 01XXXXXXXXX (bKash Merchant)
                  </p>
                  <p className="text-xs text-pink-600 mt-1">
          ...
          <div className="lg:w-1/3">
            <div className="bg-white rounded-xl shadow-md p-6 sticky top-8">
              ...
              <div className="max-h-64 overflow-y-auto mb-4">
                {cart.map((item) => (
                  <div key={item.product_id} className="flex gap-3 mb-3 pb-3 border-b">
                    <div className="w-12 h-12 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0">
                      {item.image ? (
              ...
              {shipping === 0 && subtotal > 0 && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-3 mb-4">
                  <p className="text-green-700 text-sm text-center">🎉 Free Shipping Applied!</p>
                </div>
              )}
